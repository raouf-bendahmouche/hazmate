...
- `BusinessRules` exists because some constraints are domain-specific rather than purely relational. The code currently requires a vehicle registration and a registration number (mapped to `license_number` internally) for creation, and it validates restore eligibility against the associated vehicle/company state. If those checks were only left to the UI, invalid writes could still come from API callers.
